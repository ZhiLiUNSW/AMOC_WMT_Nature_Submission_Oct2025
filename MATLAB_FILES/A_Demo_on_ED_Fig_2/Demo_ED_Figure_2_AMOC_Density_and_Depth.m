...
% % #########################################################################
% % #########################################################################
% % AMOC in density space, 2004-2023
...
% AMOCyz_ESM_005_map = squeeze(nanmean(AMOCyz_ESM_005_map(:,:,65:84,:),3));
% 
% 
% % #########################################################################
...
%      
% % #########################################################################
% % Atlnatic basin mean neutral density, 2004-2023, IAPv4.2 and Ishii
%   cd('/Users/z5195509/Documents/7_AMOC_Proxy and Variability/8_AMOC_Streamfunction_Z_convt/')
...
% 
%     
% cd('/Users/z5195509/Documents/7_AMOC_Proxy and Variability/12_Nature_R0_Submission/MATLAB_FILES/A_Demo_on_ED_Fig_2/')
% save('Demo_ED_Figure_2_AMOC_Density_and_Depth.mat','AMOCyg_ESM_005','AMOCyz_ESM_005_map','gamma_ATL_ann_map',...
%      'dens_rag_005','dep_gamma','lon_IAP','lat_IAP','basin_mask_AO')
% % #########################################################################
% % #########################################################################
...
%cd('/Users/z5195509/Documents/7_AMOC_Proxy and Variability/12_Nature_R0_Submission/MATLAB_FILES/A_Demo_on_ED_Fig_2/')
load('Demo_ED_Figure_2_AMOC_Density_and_Depth.mat','AMOCyg_ESM_005','AMOCyz_ESM_005_map','gamma_ATL_ann_map',...
     'dens_rag_005','dep_gamma','lon_IAP','lat_IAP','basin_mask_AO')
...
subplot('position',pos{41})
    % Compute AMOC streamfunction mean
      ...
      pcolor(lat_IAP,-dep_stretch,smooth2a(nanmean(AMOCyz_ESM_005_map(:,:,:),3),4,2)./1e6)
      shading interp
      ...
      % Contours
      hold on 
      [C01,h01]=contour(lat_IAP,-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,:,:),3),3,2)./1e6,-20:10:-10,'k--');
      set(h01 ,'color',[.2 .4 .6],'linewidth',2.0);
      v01=-25:5:25;
      clabel(C01,h01,v01,'labelspacing',500,'fontsize',16,'color',[.2 .2 .2])   
      hold on 
      [C012,h012]=contour(lat_IAP,-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,:,:),3),3,2)./1e6,[-2 -2],'k--');
      set(h012,'color',[.2 .4 .6],'linewidth',2.0);
      v012=-2;
      clabel(C012,h012,v012,'labelspacing',600,'fontsize',16,'color',[.2 .2 .2])   
      hold on 
      ...
      [C012,h012]=contour(lat_IAP(1:30,1),-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,1:30,4),3),1,1)./1e6,[-1 -1],'k--');
      set(h012,'color',[0    , 0.447, 0.741],'linewidth',3.0);
      v012=-1;
      clabel(C012,h012,v012,'labelspacing',600,'fontsize',16,'color',[.2 .2 .2])  
      % AABW is clear in ISHII data
      [C012,h012]=contour(lat_IAP(1:75,1),-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,1:75,4),3),1,1)./1e6,[-2 -2],'k--');
      set(h012,'color',[0    , 0.447, 0.741],'linewidth',3.0);
      v012=-2;
      clabel(C012,h012,v012,'labelspacing',600,'fontsize',16,'color',[.2 .2 .2])  
      
      hold on 
      [C02,h02]=contour(lat_IAP,-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,:,:),3),3,2)./1e6,10:5:20,'k-');
      set(h02,'color',[.8 .1 .1],'linewidth',3.0);
      v02=-25:5:25;
      clabel(C02,h02,v02,'labelspacing',1000,'fontsize',16,'color',[.2 .2 .2])   
      hold on 
      [C03,h03]=contour(lat_IAP,-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,:,:),3),3,2)./1e6,[2 2],'k-');
      set(h03,'color',[.8 .3 .3],'linewidth',2.5);
      v03=2;
      clabel(C03,h03,v03,'labelspacing',2000,'fontsize',16,'color',[.2 .2 .2])  
      
      hold on 
      [C02,h02]=contour(lat_IAP,-dep_stretch(1:end),smooth2a(nanmean(AMOCyz_ESM_005_map(:,:,:),3),3,2)./1e6,[10 10],'k-');
